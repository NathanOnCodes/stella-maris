...
class PublicacaoService:
    ...
    def buscar_publicacao_por_slug(slug: str) -> Publicacao:
        publicacao = (
            Publicacao.objects.select_related("autor", "categoria")
            .prefetch_related("tags")
            .filter(slug=slug, status=PUBLICADO)
            .filter(
                Q(data_publicacao__lte=now()) | Q(data_publicacao__isnull=True)
            )
            .first()
    ...
    def criar_publicacao(autor: User, dados: dict) -> Publicacao:
        ...
        if dados.get("categoria_id"):
            categoria = CategoriaService.buscar_categoria_por_id(
                dados["categoria_id"]
            )
    ...
    def atualizar_publicacao(
        ...
        if "slug" in dados and dados["slug"] is not None:
            ...
            "slug" in dados and dados["slug"] is None and "titulo" in dados
        ):
        ...
        if "categoria_id" in dados and dados["categoria_id"] is not None:
            publicacao.categoria = CategoriaService.buscar_categoria_por_id(
                dados["categoria_id"]
            )
    ...
    def _validar_propriedade(
        solicitante: User, publicacao: Publicacao
    ) -> None:
    ...
    def _publicacao_para_dict(publicacao: Publicacao) -> dict:
        return {
            ...
            "categoria_nome": (
                publicacao.categoria.nome if publicacao.categoria else None
            ),
            "tags": [
    ...
    def _publicacao_resumo_para_dict(publicacao: Publicacao) -> dict:
        dados = PublicacaoService._publicacao_para_dict(publicacao)
        dados.pop("conteudo")
        return dados
